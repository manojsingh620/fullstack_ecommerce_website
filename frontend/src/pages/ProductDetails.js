...
const ProductDetails = () => {
  ...
  const zoomHandleImage =useCallback((e)=>{
    setZoomImage(true)
      ...
      <div className=' min-h-[200px] flex flex-col lg:flex-row gap-4'>
        ...
        <div className='h-96 flex flex-col lg:flex-row-reverse gap-4'>
            ...
            <div className='h-[300px] w-full lg:h-96 lg:w-96 bg-slate-200 relative p-2'>
              <img src={activeImage} className='w-full h-full object-scale-down mix-blend-multiply' onMouseMove={zoomHandleImage} onMouseLeave={handleLeaveImageZoom}/>
                  ...
                  <div className='flex gap-2 lg:flex-col overflow-scroll scrollbar-hidden h-full'>
                    {
                       data?.ProductImage?.map((imgURL,index)=>{
                         return (
                             <div className='w-20 h-20 bg-slate-200 rounded  p-1 cursor-pointer' key={imgURL}>
                               <img src={imgURL} className='h-full w-full object-scale-down mix-blend-multiply' onMouseDown={()=>handleMouseEnterProduct(imgURL)} onMouseEnter={()=>handleMouseEnterProduct(imgURL)}/>
                              </div>
                            )
                        })
                    }
        ...
        {/* Product Details */}
        {
          loading ? (
            <div className='flex flex-col gap-1 w-full'>
             ...
             <div className='bg-slate-200 h-6 animate-pulse  my-1 rounded-full flex gap-2'>
            ...
            <div className='flex flex-col gap-1'>
             <h3 className='text-2xl lg:text-4xl font-medium'>{data?.ProductName}</h3>
             ...
             <div className='flex items-center gap-2 lg:text-3xl text-2xl font-medium my-1'>
              ...
              <p className=' line-through text-slate-600'>{displayINRcurrency(data?.Price)}</p>
             </div>

             <div className='flex items-center gap-3 my-2 w-full'>
              <button className='border border-yellow-600 font-bold rounded px-3 py-1 min-w-[100px] hover:bg-yellow-600 hover:text-white' onClick={(e)=>handleAddToCart(e,data?._id)}>Add to Cart</button>
              <button className='border bg-yellow-500 font-bold rounded px-3 py-1 min-w-[100px] ' onClick={(e)=>handleByProduct(e,data?._id)}>Buy</button>
             </div>
