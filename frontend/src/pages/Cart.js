...
const Cart = () => {
  ...
  const totalQty = data.reduce(
    (prevelue, currvelue) => prevelue + currvelue.Quantity,
    0
  );
  const totalPrice = data.reduce(
    (preve, currve) =>
      preve + currve?.Quantity * currve?.ProductId?.SellingPrice,
    0
  );
      ...
      <div className="flex flex-col lg:flex-row gap-10 lg:justify-evenly ">
            ...
            ? loadingCart.map((el, index) => {
                return (
                  <div
                    ...
                    className="bg-slate-200 w-full h-32 my-2 border border-slate-300 animate-pulse"
                  ></div>
                );
              })
                ...
                return (
                  ...
                  >
                    ...
                    <div className="px-4 py-2 relative ">
                      {/* delete cart product */}
                      <div
                        className="right-0 absolute text-red-600 rounded-full p-2 hover:bg-red-600 hover:text-white cursor-pointer"
                      ...
                      <div className="flex items-center gap-3 mt-2">
                        <button
                          className="p-1 text-red-600 border border-red-700 hover:bg-red-600 hover:text-white w-6 h-6 flex justify-center items-center rounded"
                          onClick={() =>
                            decreaseQnt(product?._id, product?.Quantity)
                          }
                        >
                          -
                        </button>
                        ...
                        <button
                          className="p-1 text-red-600 border border-red-700 hover:bg-red-600 hover:text-white w-6 h-6 flex justify-center items-center rounded"
                          onClick={() =>
                            increaseQnt(product?._id, product?.Quantity)
                          }
                        >
                          +
                        </button>
        ...
        {data[0] && (
          <div className="mt-5 lg:mt-6 w-full max-w-sm rounded">
            {loading ? (
              <div className="h-36 bg-slate-200 my-2 border-slate-300 animate-pulse rounded"></div>
            ) : (
